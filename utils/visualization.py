...
def draw_points(image, points, color_palette='tab20', palette_samples=16, confidence_threshold=0.5):
    ...
    kp_neck = (points[5]+points[6])/2
    # print(points[12],points[13], kp_neck)
    # print(kp_neck)
    # print(points)
    # points_f = np.append(points, kp_neck)
    ...
    for i, pt in enumerate(points):
        ...
        image = cv2.circle(image, (int(kp_neck[1]), int(kp_neck[0])), 3, tuple(colors[i % len(colors)]), -1)
